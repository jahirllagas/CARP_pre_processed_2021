...
function swap_intra(data, σ_data, solution, route_pos, Floyd_Warshall, pos1)
    ...
    for pos2 in order_services
        ...
        if pos2 != pos1
            ...
            accept_LB, links, parts, origin = lower_bound(data, σ_data, solution_av, list_route_pos, Floyd_Warshall, pos1, pos2, "swap_intra")

            if accept_LB == true
                accept = after(data, σ_data, solution_av, list_route_pos, "swap_intra", links, parts, origin)
...
function relocate_intra(data, σ_data, solution, route_pos, Floyd_Warshall, pos1)
    ...
    for pos2 in order_services
        ...
        if pos2 != pos1
            list_route_pos=[route_pos]
            accept_LB, links, parts, origin = lower_bound(data, σ_data, solution_av, list_route_pos, Floyd_Warshall, pos1, pos2, "relocate_intra")
            
            if accept_LB == true
                accept = after(data, σ_data, solution_av, list_route_pos, "relocate_intra", links, parts, origin)
...
function swap_inter(data, σ_data, solution, route_pos_1, Floyd_Warshall, pos1)
    ...
    for route_pos_2 in order_routes
        ...
        if route_pos_1 != route_pos_2
            order_services = shuffle(rng, 1:route_2.n_edges) # The deposit cannot move
            for pos2 in order_services
                new_demand = []
                ...
                if accept_demand == true
                    accept_LB, links, parts, origin = lower_bound(data, σ_data, solution_av, list_route_pos, Floyd_Warshall, pos1, pos2, "swap_inter")

                    if accept_LB == true
                        accept = after(data, σ_data, solution_av, list_route_pos, "swap_inter", links, parts, origin)
...
function relocate_inter(data, σ_data, solution, route_pos_1, Floyd_Warshall, pos1)
    ...
    for route_pos_2 in order_routes
        ...
        if route_pos_1 != route_pos_2
            order_services = shuffle(rng, 1:route_2.n_edges + 1) # The deposit cannot move
            ...
            for pos2 in order_services
                ...
                if accept_demand == true                     
                    ...
                    accept_LB, links, parts, origin = lower_bound(data, σ_data, solution_av, list_route_pos, Floyd_Warshall, pos1, pos2, "relocate_inter")

                    if accept_LB == true
                        accept = after(data, σ_data, solution_av, list_route_pos, "relocate_inter", links, parts, origin)
