...
# σ_data : (edge1.id, edge2.id] => [Mode_1_1, Mode_2_1, Mode_1_2, Mode_2_2, Lower Bound]
...
while a < 500 #(time() - startt) < 60
    ...
    while accept_move == true 
        ...
        for route_pos in route_order
            ...
            clients_order = shuffle(rng, 1:solution_av.routes[route_pos].n_edges)
                ...
                for i in order_moves
                    ...
                    if accept_move == true
                        ...
                        for route_pos in list_route_pos
                            ...
                            #cost_modes = concat_depot(data, solution.routes[route_pos], route_pos, σ_data_av)
...
#--------------------------PERTURBATION-----------------------------#    
    while M < 4 
        ...
        pos1 = rand(rng, 1:solution_av.routes[route_pos].n_edges)

        i = rand(rng, moves_ILS)

        if i == 1
            solution_av, list_route_pos = swap_intra_p(data, solution_av, route_pos, pos1)

        elseif i == 2
            solution_av, list_route_pos = relocate_intra_p(data, solution_av, route_pos, pos1)
        
        elseif i == 3
            solution_av, list_route_pos = swap_inter_p(data, solution_av, route_pos, pos1)

        elseif i == 4
            solution_av, list_route_pos = relocate_inter_p(data, solution_av, route_pos, pos1)
        end
        ...
        if solution_av.n_routes < solution.n_routes
        ...
        if M == 4
            if aceita_moves(σ_data_av, σ_data_opt, solution_av, opt_solution, Max, Min) == false
                M = 0
                solution_av = deepcopy(solution)
                σ_data_av = deepcopy(σ_data)
            else
