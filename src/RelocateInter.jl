...
function evalRelocateInter(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, routes::Vector{Route}, routes_pos::Vector{Int64}, links::Vector{Vector{Matrix{Float64}}}, parts::Vector{Vector{Tuple{Int64, Int64}}}, origins::Vector{Vector{Int64}})
    profit = Vector{Int64}()
...
function evalLBRelocateInter(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, routes::Vector{Route}, routes_pos::Vector{Int64}, rq1::Int64, rq2::Int64)

    parts1 = [(2, rq1 - 1), (rq1 + 1, length(routes[1].edges) - 1)]
    parts2 = [(2, rq2 - 1), (rq1, rq1), (rq2, length(routes[2].edges) - 1)]
    origin1 = [1, 1]
    origin2 = [2, 1, 2]
    ...
    links1, profit_lb1 = calculateLBRelocateInter(data, σ_data, sp_matrix, sequence1, parts1, routes_pos, routes[1], routes_pos[1], origin1)
    links2, profit_lb2 = calculateLBRelocateInter(data, σ_data, sp_matrix, sequence2, parts2, routes_pos, routes[2], routes_pos[2], origin2)
...
function calculateLBRelocateInter(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, sequence::Vector{Vector{Int64}}, parts::Vector{Tuple{Int64, Int64}}, routes_pos::Vector{Int64}, route::Route, route_pos::Int64, origin::Vector{Int64})
...
function moveRelocateInter(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, solution::Solution, routes_pos::Vector{Int64}, new_demand::Vector{Int64}, rq1::Int64, rq2::Int64, empty_route::Bool)
    # Effectively move the solution
    # First do RelocateInter move
    # Second update σ_data of the route
    # Third recalculate the cost of a route and the total cost
    # Fourth recalculate demand / if exists empty demand eliminate route
    service = splice!(solution.routes[routes_pos[1]].edges, rq1)
    insert!(solution.routes[routes_pos[2]].edges, rq2, service)
    if empty_route
        ...
        delete_route = splice!(solution.routes, routes_pos[1])
        delete_sigma = splice!(σ_data, routes_pos[1])
    else
...
function demandRelocateInter(data::Data, solution::Solution, routes_pos::Vector{Int64}, rq1::Int64, capacity::Int64)
    ...
    new_demand_route1 = old_demand_route1 - demand_service1
    new_demand_route2 = old_demand_route2 + demand_service1
...
function searchEmptyDemand(new_demand::Vector{Int64})
    accept = false
            ...
            if route_pos1 != route_pos2
                ...
                for rq2 in requireds
                    if rq1 != rq2
                        ...
                        if accept_demand
                            accept_lb, links, parts, origins = evalLBRelocateInter(data, σ_data, sp_matrix, routes, routes_pos, rq1, rq2)
                            if accept_lb
                                accept = evalRelocateInter(data, σ_data, sp_matrix, routes, routes_pos, links, parts, origins)
                                if accept
                                    new_solution, σ_data = moveRelocateInter(data, σ_data, sp_matrix, solution, routes_pos, new_demand, rq1, rq2, empty_route)
                                    return accept, new_solution, σ_data
                                end
