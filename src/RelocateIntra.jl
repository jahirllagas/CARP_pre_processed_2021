...
function evalRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, route::Route, route_pos::Int64, links::Vector{Matrix{Float64}}, parts::Vector{Tuple{Int64, Int64}})
    profit = 0
...
function evalLBRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, route::Route, route_pos::Int64, rq1::Int64, rq2::Int64)
    if rq2 > rq1
        if rq1 + 1 == rq2
            parts = [(2, rq1 - 1), (rq2, rq2), (rq1, rq1), (rq2 + 1, length(route.edges) - 1)]
        else
            parts = [(2, rq1 - 1), (rq1 + 1, rq2), (rq1, rq1), (rq2 + 1, length(route.edges) - 1)]
        end
    else
        if rq2 + 1 == rq1
            parts = [(2, rq2 - 1), (rq1,  rq1), (rq2, rq2), (rq1 + 1, length(route.edges) - 1)]
        else
            parts = [(2, rq2 - 1), (rq1,  rq1), (rq2, rq1 - 1), (rq1 + 1, length(route.edges) - 1)]
        end
    end

    parts = parts[deleteDepot.(parts)]
    links, profit_lb = calculateLBRelocateIntra(data, σ_data, sp_matrix, route, route_pos, parts)
...
function calculateLBRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, route::Route, route_pos::Int64, parts::Vector{Tuple{Int64, Int64}})
...
function moveRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, solution::Solution, route_pos::Int64, rq1::Int64, rq2::Int64)
    # Effectively move the solution
    # First do RelocateIntra move
    # Second update σ_data of the route
    # Third recalculate the cost of a route and the total cost
    service = splice!(solution.routes[route_pos].edges, rq1)
    insert!(solution.routes[route_pos].edges, rq2, service)
...
function perturbRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, solution::Solution, route_pos::Int64, rq1::Int64, moved::Int64)
    ...
    if rq1 != length(route.edges)
        for rq2 in requireds
            if rq1 != rq2
                new_solution, σ_data = moveRelocateIntra(data, σ_data, sp_matrix, solution, route_pos, rq1, rq2)
                moved += 1
...
function localSearchRelocateIntra(data::Data, σ_data::Vector{Matrix{σ}}, sp_matrix::Matrix{Int64}, solution::Solution, route_pos::Int64, rq1::Int64)
    accept = false
    route = deepcopy(solution.routes[route_pos])
    requireds = shuffle(2:length(route.edges) - 1)
    ...
    if rq1 != length(route.edges)
        for rq2 in requireds
            if rq1 != rq2
                accept_lb, links, parts = evalLBRelocateIntra(data, σ_data, sp_matrix, route, route_pos, rq1, rq2)
                if accept_lb
                    accept = evalRelocateIntra(data, σ_data, sp_matrix, route, route_pos, links, parts)
                    if accept
                        new_solution, σ_data = moveRelocateIntra(data, σ_data, sp_matrix, solution, route_pos, rq1, rq2)
